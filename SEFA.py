...
class SEFABLOCK(nn.Module):
    ...
    def forward(self, x, v, H, W):
        ...
        attn_windows = self.attn(x_windows, v_windows, mask=None)
...
class SEFA(nn.Module):
    def __init__(self,
        ...
        if input_dim != embed_dim:
            self.proj_e = nn.Conv2d(input_dim, embed_dim, kernel_size=3, padding=1)
        else:
            self.proj_e = nn.Identity()
        if v_dim != embed_dim:
            self.proj_q = nn.Conv2d(v_dim, embed_dim, kernel_size=3, padding=1)
        else:
            self.proj_q = nn.Identity()
        self.proj = nn.Conv2d(embed_dim, embed_dim, kernel_size=3, padding=1)
        v_dim = embed_dim

        self.sefa_block = SEFABLOCK(
            dim=embed_dim,
            num_heads=num_heads,
            v_dim=v_dim,
    ...
    def forward(self, e, q):
        ...
        q_out, H, W = self.sefa_block(q, e, Wh, Ww)
        norm_layer = getattr(self, f'norm_sefa')
